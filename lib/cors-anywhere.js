...
function proxyRequest(req, res, proxy) {
  ...
  if (locationHost.indexOf('worldcosplay.net') > 0) {
    req.method = 'GET';
    location.search = '';
    location.query = '';
    location.path = location.path.replace(search, '');
    location.href = location.href.replace(search, '');
  }
      ...
      pipe: function(proxyReq) {
        ...
        proxyReq.on = function(eventName, listener) {
          ...
          return proxyReqOn.call(this, 'response', function(proxyRes) {
            if (onProxyResponse(proxy, proxyReq, proxyRes, req, res, query)) {
              try {
                listener(proxyRes);
              } catch (err) {
 ...
 */
function onProxyResponse(proxy, proxyReq, proxyRes, req, res, query) {
  ...
  var xFinalUrl = proxyRes.headers['x-final-url'] || '';
  if (xFinalUrl.indexOf('worldcosplay.net') > 0 && query && query.indexOf('name=') >= 0) {
    var filename = query.replace('name=', '').trim() || 'worldcosplay.jpg';
    proxyRes.headers['Content-Disposition'] = `attachment; filename=${filename}; filename*=UTF-8''${unescape((filename))}`;
  }
  return true;
}
